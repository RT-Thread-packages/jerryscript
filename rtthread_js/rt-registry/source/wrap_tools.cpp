...
bool rt_js_wrap_register_global_function(const char *name, jerry_external_handler_t handler)
{
    ...
    {
        is_ok = false;
        rt_kprintf("Error: jerry_create_external_function failed!\r\n");
    ...
    {
        is_ok = false;
        rt_kprintf("Error: jerry_create_external_function has error flag! \r\n");
        jerry_release_value(global_object_val);
        jerry_release_value(reg_function);
        return is_ok;
    }
    ...
    {
        is_ok = false;
        rt_kprintf("Error: jerry_create_external_function failed: [%s]\r\n", name);
    }
...
bool rt_js_wrap_register_class_constructor(const char *name, jerry_external_handler_t handler)
{
    // Register class constructor as a global function
    return rt_js_wrap_register_global_function(name, handler);
}

bool rt_js_wrap_register_class_function(jerry_value_t this_obj, const char *name, jerry_external_handler_t handler)
{
