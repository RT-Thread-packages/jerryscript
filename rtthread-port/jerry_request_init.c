...
{
    rt_request_cbinfo_t *cb_info = (rt_request_cbinfo_t *)args;
...
void request_read_entry(void *p)
{
    ...
    if (ret_read > READ_MAX_SIZE)
    {
        ...
        rt_request_cbinfo_t *fail_info = (rt_request_cbinfo_t *)malloc(sizeof(rt_request_cbinfo_t));
        ...
        js_send_callback(rp->request_callback, fail_info, sizeof(rt_request_cbinfo_t));
        free(fail_info);
    }
    else
    {
        ...
        //do success callback
        rt_request_cbinfo_t *success_info  = (rt_request_cbinfo_t *)malloc(sizeof(rt_request_cbinfo_t));
        memset(success_info, 0, sizeof(rt_request_cbinfo_t));
        ...
        js_send_callback(rp->request_callback, success_info, sizeof(rt_request_cbinfo_t));
        free(success_info);
    }

    //do complete callback
    rt_request_cbinfo_t *complete_info  = (rt_request_cbinfo_t *)malloc(sizeof(rt_request_cbinfo_t));;
    ...
    js_send_callback(rp->request_callback, complete_info, sizeof(rt_request_cbinfo_t));
    free(complete_info);
...
void reqeuset_get_config(rt_request_config_t *config,jerry_value_t requestObj)
{
    /*get url*/
...
DECLARE_HANDLER(request)
{
    ...
    if (config.session == RT_NULL || config.response != 200)
    {
        ...
        js_send_callback(request_callback, fail_info, sizeof(rt_request_cbinfo_t));
        free(fail_info);
        //do complete callback
        rt_request_cbinfo_t *complete_info = (rt_request_cbinfo_t *)malloc(sizeof(rt_request_cbinfo_t));
        ...
        js_send_callback(request_callback, complete_info, sizeof(rt_request_cbinfo_t));
        free(complete_info);
